...
import type { FetchArgs, FetchResponse } from './types.js'

export const fetchTyped = async <T = unknown, E = unknown>(
	url: string,
	...options: FetchArgs[]
	...
	if (typeof args.query === 'object') {
		if (url.indexOf('?') >= 0)
			throw new Error(
				"Cannot fetch with both query options and a url that contains a '?'. Please solely use options.query."
			)
